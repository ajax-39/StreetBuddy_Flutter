class GuideModel {
  ...
  static List<dynamic> _parseMediaUrls(Map<String, dynamic> map) {
    // First handle the standard key 'media_urls'
    if (map['media_urls'] != null) {
      if (map['media_urls'] is String) {
        // Handle single string case (should be rare)
        return [map['media_urls']];
      } else if (map['media_urls'] is List) {
        // This is the most common case - a list of strings
        return List<dynamic>.from(map['media_urls']);
      }
    }
    // Then handle alternate key 'mediaUrls' for backward compatibility
